...
sudo pacman -Syu --noconfirm --needed || {
	echo -e "${FAIL}Failed to update package database"
	exit 1
}
...
while IFS= read -r line; do
	...
	if [[ -n $line && ! $line =~ ^\ *# ]]; then
		...
		sudo pacman -S --noconfirm --needed "$package" || {
			echo -e "${FAIL}Failed to install repo packages"
			exit 1
		}
	fi
done <./lists/pkg
...
fc-cache -f -v || {
	echo -e "${FAIL}Failed to register fonts"
	exit 1
}
...
install_paru || {
	echo -e "${FAIL}Failed to install paru"
	exit 1
}
...
sudo systemctl enable --now bluetooth.service || {
	echo -e "${FAIL}Failed to start bluetooth service"
	exit 1
}
...
function setup_zsh() {
	mkdir -p ~/.zsh
	rm -rf ~/.zsh/zsh-autosuggestions ~.zsh/zsh-syntax-highlighting ~/.zsh/zsh-history-substring-search || true
	git clone https://github.com/zsh-users/zsh-autosuggestions ~/.zsh/zsh-autosuggestions
	git clone https://github.com/zsh-users/zsh-syntax-highlighting.git ~/.zsh/zsh-syntax-highlighting
	git clone https://github.com/zsh-users/zsh-history-substring-search ~/.zsh/zsh-history-substring-search
}
setup_zsh || {
	echo -e "${FAIL}Failed to setup zsh"
	exit 1
}
...
sudo chsh -s $(which zsh) && chsh -s $(which zsh) || {
	echo -e "${FAIL}Failed to set shell to ZSH"
	exit 1
}
...
function links() {
	rm ~/.bashrc ~/.bash_profile || true
	sudo ln -s ~/dotfiles/sddm.conf /etc/sddm.conf
	stow .
}
links || {
	echo -e "${FAIL}Failed to symlink configurations"
	exit 1
}
...
enable_services || {
	echo -e "${FAIL}Failed to enable services"
	exit 1
}
...
sudo systemctl enable sddm || {
	echo -e "${FAIL}Failed to enable sddm"
	exit 1
}
echo -e "${DONE}Enabled sddm successfully"

echo -e "${DONE}Finished running setup script!"
